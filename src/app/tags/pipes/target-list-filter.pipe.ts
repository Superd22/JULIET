...
export class TargetListFilterPipe implements PipeTransform {
  ...
  private filterByType(target: TagTarget): boolean {
    if (this.filters.type != null) {
      ...
      if (typeof this.filters.type == typeof "string") return target.type.toLowerCase().indexOf(this.filters.type.toLowerCase()) > -1;
      // We want any of the types in our array
      else {
        let found = false;
        for (let i = 0; i < this.filters.type.length; i++) {
          if (target.type.toLowerCase().indexOf(this.filters.type[i].toLowerCase()) > -1) {
            found = true;
            break;
          }
        }
